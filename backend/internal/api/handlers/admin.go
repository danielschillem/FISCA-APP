...
func (h *AdminHandler) Stats(w http.ResponseWriter, r *http.Request) {
	...
	h.DB.QueryRow(ctx, `SELECT COUNT(*) FROM users WHERE plan IN ('starter','physique_starter') AND role != 'super_admin'`).Scan(&s.PlanStarter)
	h.DB.QueryRow(ctx, `SELECT COUNT(*) FROM users WHERE plan IN ('pro','physique_pro','moral_team') AND role != 'super_admin'`).Scan(&s.PlanPro)
	h.DB.QueryRow(ctx, `SELECT COUNT(*) FROM users WHERE plan IN ('enterprise','moral_enterprise') AND role != 'super_admin'`).Scan(&s.PlanEnterprise)
