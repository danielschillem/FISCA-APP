...
function IUTSCalc() {
    ...
    const r = calcEmploye(form as Parameters<typeof calcEmploye>[0]);
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        ...
                        <div className="flex gap-2">
                            {['Cadre', 'Non-cadre'].map((c) => (
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${form.categorie === c
                        ...
                        <div className="flex gap-2">
                            {[['CNSS', '5,5 %'], ['CARFO', '6 %']].map(([v, l]) => (
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${form.cotisation === v
            ...
            <Card title="Résultat — Détail fiscal CGI 2025">
                ...
                <ResultRow label={`Cotisation ${form.cotisation}`} value={`- ${fmt(r.cotSoc)}`} sub={`${(r.cotSoc / r.remBrute * 100).toFixed(1)} %`} />
                ...
                <ResultRow label="Abatt. familial (Art.113)" value={`+ ${fmt(r.abattFam)}`} sub={`${form.charges} charge(s) — ${(r.abattFam / (r.iutsBrut || 1) * 100).toFixed(0)} %`} />
...
// ─── IRF ────────────────────────────────────────────────────
function IRFCalc() {
    const [loyer, setLoyer] = useState(300000);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <Card title="CME — Micro-Entreprises (CGI 2025 Art. 533)">
                <div className="space-y-3">
                    <Input label="Chiffre d'affaires annuel (FCFA)" type="number" value={String(ca)} onChange={(e) => setCA(+e.target.value)} suffix="FCFA" />
...
function PatenteCalc() {
    const [ca, setCA] = useState(20000000);
    const [locatif, setLocatif] = useState(500000);
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <Card title="Patentes (CGI 2025 Art. 237–240)">
                <div className="space-y-3">
                    <Input label="CA HT exercice précédent (FCFA)" type="number" value={String(ca)} onChange={(e) => setCA(+e.target.value)} suffix="FCFA" />
                    <Input label="Valeur locative locaux prof. (FCFA)" type="number" value={String(locatif)} onChange={(e) => setLocatif(+e.target.value)} suffix="FCFA" />
                </div>
            </Card>
...
// ─── IS / MFP ───────────────────────────────────────────────
function ISCalc() {
    const [ca, setCA] = useState(100000000);
    const [benefice, setBenefice] = useState(10000000);
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <Card title="IS / MFP — Impôt sur les Sociétés (CGI 2025 Art. 42 — 27,5 %)">
                <div className="space-y-3">
                    <Input label="CA HT (FCFA)" type="number" value={String(ca)} onChange={(e) => setCA(+e.target.value)} suffix="FCFA" />
                    <Input label="Bénéfice imposable (FCFA)" type="number" value={String(benefice)} onChange={(e) => setBenefice(+e.target.value)} suffix="FCFA" />
