...
function IRFContent() {
    ...
    return (
        <div className="max-w-3xl space-y-6">
            <Card title="IRF � Imp�t sur les Revenus Fonciers">
                <p className="text-xs text-gray-500 mb-4">CGI 2025 � Art. 121-126 � Abattement 50 % � Taux progressif 18 % / 25 %</p>
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Ann�e fiscale</label>
                ...
                {createMut.isSuccess && <p className="text-xs text-green-600 mt-2">D�claration enregistr�e ?</p>}
            </Card>

            {result && (
                <Card title="R�sultat IRF">
            ...
            <Card title={`Historique IRF`}>
                <div className="flex items-center gap-3 mb-3">
                    <label className="text-xs text-gray-600">Ann�e :</label>
                ...
                {isLoading ? <Spinner /> : history.length === 0 ? (
                    <p className="text-xs text-gray-400 py-4 text-center">Aucune d�claration IRF pour {annee}</p>
                ) : (
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="text-gray-500 border-b border-gray-100">
                                    ...
                                    <th className="text-right py-2">IRF d�</th>
                                    ...
                                    <tr key={d.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-2 text-center">
                                            <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${d.statut === 'declare' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                {d.statut === 'declare' ? 'D�clar�' : 'Brouillon'}
                                            </span>
                                        </td>
            ...
            <Card title="Base l�gale">
                <ul className="text-xs text-gray-500 space-y-1">
                    ...
                    <li>� Art. 124 : Taux progressif 18 % jusqu'� 100 000 FCFA base nette, 25 % au-del�</li>
                    <li>� D�claration annuelle avant le 30 avril</li>
                </ul>
            </Card>
        </div>
    );
    ...
    return (
        <div className="flex items-center justify-center min-h-[50vh]">
            <div className="text-center max-w-xs">
                ...
                <h2 className="text-xl font-bold text-gray-900 mb-2">Fonctionnalit� <span className="text-green-600">Pro</span></h2>
                <p className="text-gray-500 text-sm">Passez au plan Pro pour acc�der au module IRF.</p>
            </div>
        </div>
    );
}
